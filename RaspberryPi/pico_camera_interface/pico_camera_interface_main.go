...
func convertPixelsToDegree(coords []int32) (int16, int16) {

	// Find center of target
	axis1_center := float32(coords[0]+coords[2]) / 2
	axis2_center := float32(coords[1]+coords[3]) / 2
...
func main() {
	...
	for signal := range c {
		...
		if signal.Name == "org.cacophony.thermalrecorder.Tracking" {
			// // Extract data
			what := signal.Body[0]
			confidence := signal.Body[1]
			coords := signal.Body[2].([]int32)
			tracking := signal.Body[3]
